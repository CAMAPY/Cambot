...
class translate:
    ...
    async def translate(self, ctx, to_language, *, msg):
        ...
        async with self.client.session.get("https://gist.githubusercontent.com/astronautlevel2/93a19379bd52b351dbc6eef269efa0bc/raw/18d55123bc85e2ef8f54e09007489ceff9b3ba51/langs.json") as resp:
        ...
        for entry in lang_codes:
            if to_language in lang_codes[entry]["name"].replace(";", "").replace(",", "").lower().split():
                ...
                real_language = True
        if real_language:
            async with self.client.session.get("https://translate.google.com/m",
            ...
            await ctx.send(self.client.client_prefix + "That's not a real language.")
